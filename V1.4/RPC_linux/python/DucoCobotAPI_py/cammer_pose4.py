...
def callback(data):
    ...
    if(flag==90):
        ...
        #设置目标mark2cammer
        #mark2cammer = ro_rt_T(0.06+0.0786,0.016+0.0644,0.4-0.0275,1,0,0,0)
        mark2cammer = ro_rt_T(0.06+0.0786,0.016+0.0644,0.4-0.0275,1,0,0,0)
    ...
    if(move_flag==10):
        ...
        if rtde_r.getActualTCPForce()[2]>45:#受到得阻力大于45则不再下降
             rtde_c.stopL
        for  i in range(10):
            command = genCommand(1,1,50,0,255,1)#松开夹抓
            pub.publish(command)
            rospy.sleep(0.1)
        get_current_pose = rtde_r.getActualTCPPose()
        get_current_pose[2]=get_current_pose[2]+0.1#上移
        rtde_c.moveL(get_current_pose,0.5,0.3)

        for  i in range(10):
            command = genCommand(1,1,255,0,255,1)#合并夹抓下压
            pub.publish(command)
            rospy.sleep(0.1)
        if rtde_r.getActualTCPForce()[2]>60:#阻力大于60则不再下降
             rtde_c.stopL
        # sleep(2)
    ...
    #     get_current_pose = rtde_r.getActualTCPPose()
    #     get_current_pose[2]=get_current_pose[2]-0.035
    #     rtde_c.moveL(get_current_pose,0.5,0.3)
    ...
    #     sleep(2)
        # for  i in range(10):
        #     command = genCommand(1,1,50,0,255,1)
        #     pub.publish(command)
        #     rospy.sleep(0.1)

    #     get_current_pose[2]=get_current_pose[2]+0.1
    #     rtde_c.moveL(get_current_pose,0.5,0.3)

    #     for  i in range(10):
    #         command = genCommand(1,1,255,0,255,1)
    #         pub.publish(command)
    #         rospy.sleep(0.1)

    #     speed = [0, 0,-0.100, 0, 0, 0]
    #     rtde_c.moveUntilContact(speed)
    #     if rtde_r.getActualTCPForce()[2]>10:
    #         rtde_c.stopL
...
if __name__ == "__main__":
    ...
    #瓶盖位置
    PG_pose=[-0.49079,-0.34728,0.26722,1.935,2.504,0.039]
    rtde_c.moveL([PG_pose[0],PG_pose[1],PG_pose[2],PG_pose[3],PG_pose[4],PG_pose[5]],0.5,0.3)
    for  i in range(2):
        command = genCommand(1,1,180,0,120,1)#第三位原来200，第四位原来150
        pub.publish(command)
        rospy.sleep(0.1)
    ...
    get_current_pose = rtde_r.getActualTCPPose()
    get_current_pose[2]=get_current_pose[2]+0.2#原来0.3
    rtde_c.moveL(get_current_pose,0.5,0.3)
